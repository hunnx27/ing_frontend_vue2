...
msg "================================================================================="
msg "DEPLOY ONZ FRONT-END VueJS SERVER"

msg "\nScript executed from :"
pwd

COUNT=$(docker images "onz-fe-vue:${now}*" | wc -l)
VERSION_NUMBER="${now}-${COUNT}"

echo ${VERSION_NUMBER}

docker_build=$(time docker build -t onz-fe-vue:${VERSION_NUMBER} .)
...
msg "docker rm onz-fe-vue old version"
docker rm --force onz-be

msg "docker run"
docker_run=$(docker run -p 9100:80 --name onz-fe-vue onz-fe-vue:${VERSION_NUMBER})
if ! [ "${docker_run}" ]; then
  die "Docker Run FAILED"
fi
msg "${GREEN}Docker container run success${NOFORMAT}"

msg "ONZ FRONT-END VueJS SERVER TAG : ${VERSION_NUMBER}"
msg "================================================================================="
